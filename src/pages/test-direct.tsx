...
export default function TestDirectPage() {
  ...
  useEffect(() => {
    const fetchJournals = async () => {
      try {
        console.log('Fetching published journals from direct test endpoint...');
        const response = await fetch('/api/test/published-journals');
        const data = await response.json();
        
        console.log('Raw API response from direct test endpoint:', data);
        setRawResponse(data);
        
        if (data.journals) {
          console.log('Found journals from direct test endpoint:', data.journals);
          setJournals(data.journals);
        } else {
          console.log('No journals found in direct test endpoint response');
          setError('No journals found in response');
        }
      } catch (err) {
        console.error('Error fetching journals from direct test endpoint:', err);
  ...
  return (
    <Layout title="Test Direct Published Journals" description="Testing direct published journals endpoint">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold mb-6">Test Direct Published Journals Endpoint</h1>
